         ...
         try:
          for s in HLSStream.parse_variant_playlist(self.session, hls_url, headers=headers).items():
           ...
           if mod == 'PLAY':
            ...
            print
            command = ('start ffplay -i {} -infbuf -autoexit -x 640 -y 480 -window_title "{}"'.format(hls_url,filename))
            os.system(command)
           ...
           if mod == 'REC':
            ...
            print
            command = ('start ffmpeg -i {} -c:v copy -c:a aac -b:a 160k {}'.format(hls_url,pf))
            os.system(command)
            time.sleep(1)    # pause 1 second
            print(colored(" => END <= ", "yellow","on_blue"))
            sys.exit()
